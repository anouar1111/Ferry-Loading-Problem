...
class Main{
  ...
  private boolean[][] visited; //big table qui sauvegarde les etats visités ou pas
  ...
  void BacktrackSolve(int currK , int currS){
      ...
      if(currK < n){ //il y a encore des voitures à considérer
        if(length.get(currK)<=currS && visited[currK+1][(currS-length.get(currK))]== false ){ //possible d'ajouter la voiture suivante a gauche + l'etat non visité
            currX[currK]=1; //on met la voiture a gauche
            newS = currS - length.get(currK);
            BacktrackSolve(currK+1, newS);
            visited[currK+1][newS]= true; //on met l'etat comme étant visité
        }
        if((SPrime >= length.get(currK)) && visited[currK+1][currS]== false){//possible d'ajouter la voiture suivante a droite + l'etat non visité
          currX[currK] = 0; //on met la voiture a droite
          SPrime = SPrime - length.get(currK);
          BacktrackSolve(currK+1, currS);
          SPrime = SPrime + length.get(currK);
          visited[currK+1][currS]= true; //on met l'etat comme étant visité
        }
      }
  }
  ...
  public static void main (String args[]){
    ...
    for(int i=0 ; i < main.nbCases; i++){//on refait le meme processus pour chaque cas
      ...
      main.L= input.nextInt()*100;//conversion vers le centimetre
      ...
      main.visited = new boolean[main.n+1][main.L+1];
      main.bestK = -1;
      main.newS = -1;
